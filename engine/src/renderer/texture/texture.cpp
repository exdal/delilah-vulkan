...
Texture *texture::create(const char *file, bool nearest, WrappingStyle wrap) {
    ...
    glTexParameteri(texture->id, GL_TEXTURE_WRAP_S, wrap_type);
    glTexParameteri(texture->id, GL_TEXTURE_WRAP_T, wrap_type);

    uint32_t filter_type = (nearest) ? GL_NEAREST : GL_LINEAR;

    glTexParameteri(texture->id, GL_TEXTURE_MIN_FILTER, filter_type);
    glTexParameteri(texture->id, GL_TEXTURE_MAG_FILTER, filter_type);
